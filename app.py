...
def check(email):
    url = "https://u1a74w8ljj.execute-api.us-east-2.amazonaws.com/plasma/registration/getdata?email=" + email
...
def register():
    ...
    if ('errorType' in check(x[1])):
        url = "https://u1a74w8ljj.execute-api.us-east-2.amazonaws.com/plasma/registration?" + params
...
@app.route('/stats')
def stats():
    url = "https://u1a74w8ljj.execute-api.us-east-2.amazonaws.com/plasma/registration/getdata/getbloodgroupdata"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    # print(bloodgrp)
    url = "https://u1a74w8ljj.execute-api.us-east-2.amazonaws.com/plasma/requestonbloodgroup?blood=" + bloodgrp
    status = requests.request("GET", url)
    a = status.json()
    print(a)
    phone = []
    for i in a:
        url = "https://www.fast2sms.com/dev/bulk?authorization=3IfHvrRSaKdzw1enBOMG5kEbLm69iDY2Tl0sJucqgoxp4UZQyXWezyB7Mf09aZuwIgN3JY1cxQkpslD8&sender_id=FSTSMS&message=Need plasma of your blood group&language=english&route=p&numbers=" + str(
            i['phone'])
